...
if($is_paid) {
    ...
    foreach ($basket_contents as $item) {
        if(!$db_handler->getStockLevelOfItem($item["basket_productid"]) >= $item["entry_quantity"] ) {
            header("Location: ../Pages/cart.php?error=outOfStock&item_id=" . $item["basket_productid"]);
            exit;
        }
    }
